...
function resolveImageUrl(imageUrl: string | undefined, podcastId: string): string {
  if (!imageUrl || imageUrl === "") {
    return `${getBackendBase()}image/default.png`;
  }
  if (imageUrl.startsWith('http') || imageUrl.startsWith('file://') || imageUrl.startsWith('data:') || imageUrl.startsWith('/') || imageUrl.startsWith('blob:')) {
    return imageUrl;
  }
  // Relative path like "image/daily.png" - convert to absolute backend URL
  return `${getBackendBase()}${imageUrl}`;
}
...
const PodcastItem = ({ 
  ...
  const [coverImageUrl, setCoverImageUrl] = useState<string>(podcast.image_url);
  
  // Load the cover image from local storage when component mounts
  useEffect(() => {
    const loadCoverImage = async () => {
      const url = await loadAsset(podcast.image_url, 'image');
      if (url) {
  ...
  return (
    ...
    >
      <div className="relative aspect-square">
        <Image
          src={resolveImageUrl(coverImageUrl, podcast.id)}
          alt={podcast.title || ''}
          width={300}
          height={300}
      ...
      try {
        ...
        const downloadsWithAssets = await Promise.all(
          downloads.map(async (podcast) => {
            ...
            if (podcast.image_url) {
              const coverUrl = await loadAsset(podcast.image_url, 'image');
              if (coverUrl) {
                podcast.image_url = coverUrl;
              }
            }
      ...
      <header className="bg-white/10 backdrop-blur-md shadow-lg">
        <div className="w-full px-2 sm:px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/downloads" className="text-amber-400 font-medium">Downloads</Link>
