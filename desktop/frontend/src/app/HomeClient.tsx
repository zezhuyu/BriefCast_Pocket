...
const parseDateString = (timestamp: string | number): Date => {
  ...
  if (typeof timestamp === 'number') {
    // Timestamps from backend are in milliseconds (Date.now()). Values > 1e10 are ms; < 1e10 are legacy seconds.
...
export default function Home() {
  ...
  // Derived display values: override with transition host card while clip plays
  const displayImage = (isPlayingTransition && transitionDisplay?.image_url) ? transitionDisplay.image_url : currentPodcast?.image_url ?? '';
  const displayTitle = (isPlayingTransition && transitionDisplay?.title) ? transitionDisplay.title : currentPodcast?.title ?? '';
  ...
  // isTauri is always false in Electron+Next.js mode (no Tauri runtime)
  useEffect(() => {
    setIsTauri(false);
  }, []);
  ...
  useEffect(() => {
    // While a transition clip is playing, show its inline text with proportional timing.
    if (isPlayingTransition && transitionDisplay?.text) {
      const durSecs = transitionDisplay.duration_seconds || 10;
      const chunks = transitionDisplay.text
        .split(/(?<=[.!?])\s+/)
        .map((s: string) => s.trim())
        .filter(Boolean);
      const wordCounts = chunks.map((c: string) => c.split(/\s+/).filter(Boolean).length);
      const totalWords = wordCounts.reduce((a: number, b: number) => a + b, 0);
      let cursor = 0;
      const data = chunks.map((text: string, i: number) => {
        const t = cursor;
        cursor += totalWords > 0 ? (wordCounts[i] / totalWords) * durSecs : durSecs / chunks.length;
        return { time: t, text };
      });
    ...
    // Restore normal transcript when transition ends
    if (!isPlayingTransition && transitionDisplay === null && currentPodcast?.transcript_url) {
      // fall through to loadTranscript below
    }
    ...
    const loadTranscript = async (url: string, attempt: number = 1) => {
      ...
      // Skip placeholder URLs - podcast is still generating
      if (url.endsWith("/starting.lrc") || url === "transcript/starting.lrc") {
        console.log("[HomeClient] Skipping transcript load - placeholder URL:", url);
        setTranscriptData([]);
        return;
      }
      try {
  ...
  }, [currentPodcast, isPlayingTransition, transitionDisplay])
  ...
  useEffect(() => {
    ...
    const fixUrls = async () => {
      ...
      // desktop-ts returns file:// or data: URLs — use as-is, no /files/ prefix needed
    };
  ...
  const regenerateCurrentAudio = async () => {
    if (!currentPodcast || regenerating) return;
    setRegenerating(true);
    setAudioError(null);
    try {
      const normalizedBase = getBackendBase();
      const isDaily = Boolean((currentPodcast as any).is_daily) || !podcastId || currentPodcast.id.startsWith('daily-');

      const postWithFallback = async (primaryPath: string, fallbackPath: string, body?: Record<string, unknown>) => {
        const primary = await fetch(`${normalizedBase}${primaryPath}`, {
          method: 'POST',
        ...
        return fetch(`${normalizedBase}${fallbackPath}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: body ? JSON.stringify(body) : undefined,
          cache: 'no-cache'
      ...
      if (isDaily) {
        const response = await postWithFallback('api/podcast/force-daily', 'podcast/force-daily');
        if (!response.ok) {
          throw new Error(`Failed to regenerate daily podcast: ${response.status}`);
        }
      ...
      const response = await postWithFallback(
        'api/podcast/generate-audio',
        'podcast/generate-audio',
        { recommendationId: currentPodcast.id }
      );
      if (!response.ok) {
        throw new Error(`Failed to regenerate podcast: ${response.status}`);
      }
      const data = await response.json();
      if (data && data.audio_url) {
        setCurrentPodcast(data);
                  ...
                  <div className="flex-grow">
                    <div className="flex flex-col gap-6 mb-6">
                      ...
                      <div className="relative group w-full aspect-square overflow-hidden rounded-xl shadow-xl">
                        ...
                        <div className="absolute bottom-0 left-0 p-6 text-white">
                          ...
                          {!isPlayingTransition && currentPodcast.link?.startsWith('http') && (
                            <a 
                    ...
                    <div className="space-y-6 p-4 rounded-xl">
                      ...
                      <div className="flex justify-center items-center gap-6">
                        ...
                        {/* Regenerate button */}
                        <button
                          className={`text-gray-600 dark:text-gray-300 hover:text-amber-500 dark:hover:text-amber-400 transition-colors ${regenerating ? 'opacity-70 cursor-wait' : ''}`}
                          onClick={regenerateCurrentAudio}
                          disabled={regenerating}
                          aria-label="Regenerate podcast audio"
                          title="Regenerate podcast audio"
                        >
                          <svg className={`w-6 h-6 ${regenerating ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    ...
                    <div className="overflow-y-auto" ref={mobileTranscriptRef} style={{ height: playerHeight ? `${playerHeight - 80}px` : '500px' }}>
                      <div className="space-y-4">
                        {/* Show generating message when podcast is selected but transcript not ready */}
                        {currentPodcast && transcriptData.length === 0 && (
                          <div className="flex flex-col items-center justify-center h-full py-12">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-500 mb-4"></div>
                            <p className="text-gray-300 text-lg">Generating podcast...</p>
