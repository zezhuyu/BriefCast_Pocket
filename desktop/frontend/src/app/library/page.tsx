...
const parseDateString = (timestamp: string | number | undefined | null): Date => {
...
const toDate = (timestamp: string | number | undefined | null): string => {
...
const resolveLibraryHistoryImageUrl = (rawUrl?: string, id?: string) => {
  const fallback = backendUrl("image/default.png");
  ...
  return cleanPath ? backendUrl(cleanPath) : fallback;
};

const resolveLibraryPodcastImageUrl = (rawUrl?: string, imageResource?: string) => {
  const fallback = backendUrl("image/default.png");
  if (rawUrl && rawUrl.trim()) {
    ...
    const cleanPath = normalized.startsWith("/") ? normalized.slice(1) : normalized;
    if (cleanPath) return backendUrl(cleanPath);
  }
  if (imageResource && imageResource.trim()) {
    const cleanResource = imageResource.trim().replace(/^\/+/, "");
    return backendUrl(`image/${cleanResource}`);
  }
  return fallback;
};
...
export default function LibraryPage() {
  ...
  const searchPodcasts = async () => {
    ...
    const response = await fetch(backendUrl(`api/news/search?query=${encodeURIComponent(searchTerm)}&mode=hybrid`), {
      method: "GET",
    });
    const data = await response.json();
  ...
  const createSummary = async () => {
    ...
    try {
      ...
      const response = await fetch(backendUrl("api/podcast/generate-summary"), {
        method: 'POST',
        headers: {
    ...
    return (
      ...
      >
        <div className="relative aspect-square">
          <Image
            src={resolveLibraryPodcastImageUrl(podcast.image_url, podcast.imageResource)}
  ...
  const renderHistoryItem = (item: any) => (
    <div 
      key={item.id || `${item.recommendation_id || item.podcast_id || item.title}-${item.listened_at || item.published_at || 0}`}
      ...
      onClick={() => handlePodcastClick(item.recommendation_id || item.podcast_id || item.id)}
    >
      <div className="relative h-32">
        <Image
          src={resolveLibraryHistoryImageUrl(item.image_url, String(item.id))}
          alt={item.title}
          fill
          unoptimized
          sizes="(max-width: 768px) 100vw, 50vw"
      ...
      <header className="bg-white/10 backdrop-blur-md shadow-lg">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/library" className="text-amber-400 font-medium">Library</Link>
                  ...
                  {trendingItems.map((item) => (
                    ...
                    >
                      <div className="relative w-10 h-10 flex-shrink-0">
                        <Image
                          src={resolveLibraryPodcastImageUrl(item.image_url, item.imageResource)}
