...
const resolveHistoryImageUrl = (rawUrl?: string, id?: string) => {
  ...
  // legacy backend returns relative paths like "image/default.png"
...
const groupHistoryByMonth = (history: HistoryItem[]): Record<string, HistoryItem[]> => {
  ...
  history.forEach(item => {
    const ts = item.listened_at as number;
    let date = new Date(ts > 1e10 ? ts : ts * 1000);
...
export default function HistoryPage() {
  ...
  const renderHistoryItem = (item: {
    ...
    <div 
      ...
      onClick={() => handlePodcastClick(item.recommendation_id || item.id)}
    >
      <div className="relative aspect-square">
        <Image
          src={resolveHistoryImageUrl(item.image_url, String(item.id))}
