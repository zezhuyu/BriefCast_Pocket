...
export const convertLRCToTranscriptData = async (lrcFilePath: string) => {
    try {
      const normalizedPath = typeof lrcFilePath === "string" ? lrcFilePath.trim() : "";
      if (!normalizedPath) {
        return [];
      }
      ...
      if (!response.ok) {
        console.warn(`Failed to fetch LRC file: ${response.status} (${normalizedPath})`);
        return [];
      }
