...
const DEFAULT_SETTINGS: AppSettings = {
  ...
  preferences: { topics: [], region: "US", language: "en" },
};
...
export default function ServerConfigPage({ isTauri }: { isTauri: boolean }) {
  ...
  const fetchSettings = useCallback(async (quiet = false) => {
    ...
    try {
      const { data } = await axios.get<AppSettings>(`${BASE}settings`);
      setSettings((prev) => {
        // Merge to avoid overwriting local edits when hotloading quietly
  ...
  useEffect(() => {
    const onVisible = () => {
      if (document.visibilityState === "visible" && activeTab === "settings") {
        fetchSettings(true).then((fresh) => {
          if (fresh) setSettings({ ...DEFAULT_SETTINGS, ...fresh });
        });
      }
    };
  ...
  const saveSettings = async () => {
    ...
    setSaveMsg(null);
    try {
      const { data } = await axios.post<AppSettings>(`${BASE}settings`, settings);
  ...
  const handleRssSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${BASE}settings`, { ...data, sources: { ...data.sources, rssFeeds: feeds } });
      await fetchRssLinks();
      resetRssForm();
    } catch (err: any) {
  ...
  const handleForceRefresh = async () => {
    ...
    try { await axios.post(`${BASE}news/sync`); } catch {}
